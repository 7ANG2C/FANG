...
internal class FundPDFViewModel(
    ...
    private data class Mediator(val year: String, val totalFund: String)
    ...
    init {
        viewModelScope.launch {
            requestTrigger.flatMapLatest { request ->
                if (request == null) {
                    emptyFlow()
                } else {
                    sheetRepository.workSheets
                        .mapLatest { workSheets ->
                            workSheets?.sheetFund()?.values?.filter {
                                it.millis in request.startMillis..request.endMillis
                            }?.sortedByDescending { it.id }?.let { funds ->
                                val ymFund =
                                    funds.map {
                                        MFund(
                                            selected = false,
                                ...
                                if (ymFund.isEmpty()) {
                                    ...
                                    val mediators =
                                        ymFund.groupBy { it.year }.map { (k, v) ->
