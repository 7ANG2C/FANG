...
internal class SalaryViewModel(private val repository: SheetRepository) : ViewModel() {
        ...
        viewModelScope.launch {
            repository
                ...
                .mapLatest { workSheets ->
                    ...
                    workSheets?.sheetAttendance()?.values.orEmpty().sortedByDescending { it.id }
                        .groupBy {
                            ...
                            val months =
                                yearAtts.groupBy {
                                    ...
                                    val employeeSalaries =
                                        ...
                                        }.groupBy { it.employeeId }.map { (employeeId, mediators) ->
                                            val employee = employees.find { it.id == employeeId }
                                            val salaries = employee?.salaries.orEmpty()
                                            val salary =
                                                mediators.mapNotNull { m ->
                                                    salaries.find {
                                                        m.attMillis >= it.millis
                                                    }?.salary?.let {
                                                        it to m.attFactor
                                                    }
                                                }.takeIf { it.isNotEmpty() }?.sumOf {
                                                    it.first * it.second
                                                }
                                            EmployeeSalary(
                                    ...
                                    MonthSalary(
                                        ...
                                        salary =
                                            employeeSalaries.mapNotNull { it.salary }
