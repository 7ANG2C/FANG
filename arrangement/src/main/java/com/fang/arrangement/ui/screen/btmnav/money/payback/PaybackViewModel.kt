...
internal class PaybackViewModel(
            ...
            execute {
                sheetRepository.update(
                    Payback(
                        ...
                        payback = edit.payback!!.toInt(),
                        millis = edit.millis!!,
                        records = edit.records.map { PaybackRecord(it.millis!!, it.payback!!.toInt(), it.remark.takeIfNotBlank?.trim()) },
                        remark = edit.remark.takeIfNotBlank?.trim(),
                    ),
                )
            }
    ...
    fun delete(id: String) {
        execute {
            sheetRepository.delete<Payback>(id)
        }
    }
    ...
    fun bossInsert(edit: BossEdit) {
        if (edit.savable) {
            bossExecute {
                sheetRepository.insert(Boss(System.currentTimeMillis(), edit.name.orEmpty().trim(), 0))
            }
        }
    }
    ...
    fun bossUpdate(editBundle: BossEditBundle) {
        ...
        if (current != null && edit.savable && editBundle.anyDiff) {
            bossExecute {
                sheetRepository.update(Boss(current.id, edit.name.orEmpty().trim(), 0))
            }
        }
    }
