...
object NumberFormat {
    ...
    const val UNSPECIFIED_DECIMAL_SIZE = -1

    /**
     * 格式化數字格式
     * @param string 當可以轉換成 [BigDecimal] 才進行 format ，否則回傳 invalidText
     * @param decimalCount 小數點保留位數，若 < [UNSPECIFIED_DECIMAL_SIZE] 拋出 [IllegalArgumentException]
     * @param autoFillZero 是否自動補 0，當 true，數字 =0 時，自動補 0
     * @param thousandSeparator 有否有千位分隔符
     */
    operator fun invoke(
        string: String?,
        decimalCount: Int = UNSPECIFIED_DECIMAL_SIZE,
        ...
        require(decimalCount >= UNSPECIFIED_DECIMAL_SIZE)
        val bigDecimal = string?.toBigDecimalOrNull()
        return if (bigDecimal != null) {
            var dot = ""
            val decimalSize = if (decimalCount == UNSPECIFIED_DECIMAL_SIZE) {
            ...
            val integer = if (thousandSeparator) {
                ",##${placeholder}"
            } else {
                placeholder
            }.takeIf { autoFillZero || bigDecimal.abs() > BigDecimal.ONE }.orEmpty()
            val decimal = if (decimalSize > 0) {
